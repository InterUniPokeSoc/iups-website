...
function readFromCache(slices) {
  ...
  if (slices) {
    return _v.default.deserialize((0, _fsExtra.readFileSync)(reduxWorkerSlicesPrefix(reduxCacheFolder) + (0, _gatsbyCoreUtils.createContentDigest)(slices)));
  }
...
function prepareCacheFolder(targetDir, contents, slices) {
  if (slices) {
    (0, _fsExtra.writeFileSync)(reduxWorkerSlicesPrefix(targetDir) + (0, _gatsbyCoreUtils.createContentDigest)(slices), _v.default.serialize(contents));
    return;
...
function writeToCache(contents, slices) {
  ...
  prepareCacheFolder(tmpDir, contents, slices); // Replace old cache folder with new. If the first rename fails, the cache
